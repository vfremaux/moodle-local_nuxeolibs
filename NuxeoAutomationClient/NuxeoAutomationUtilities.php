...
class NuxeoRequest {
     ...
     *
     * @param $schema :
     *            name the schema you want to obtain
     * @author Arthur GALLOUIN for NUXEO agallouin@nuxeo.com
     */
        ...
        if ($requestVarVallue !== NULL) {
            if ($this->iterationNumber === 0) {
                $this->finalRequest = array (
                        $requestType => array (
                                $requestContentOrVarName => $requestVarVallue 
                        ) 
                );
            } else if ($this->iterationNumber === 1) {
                $this->finalRequest [$requestType] = array (
                        $requestContentOrVarName => $requestVarVallue 
                );
    ...
    private function multiPart() {
        ...
        $data = "--" . $boundary . "\r\n" . $requestheaders . $this->finalRequest . "\r\n" . "\r\n";
        ...
        $head = 'Accept: application/json+nxentity, */*' . "\r\n" . 'Content-Type: multipart/related;boundary="' . $boundary . '";type="application/json+nxrequest";start="request"' . "\r\n" . $this->X_NXVoidOperation;
        
        $headerstsend = $head . "\r\n" . $this->headers;
        
        $final ['http'] ['header'] = $headerstsend;
        error_log ( " header =  " . $final ['http'] ['header'], 3, "/var/tmp/errors_portfolio.log" );
     ...
     *
     * @param $adresse :
     *            contains the path of the file to load
     * @param $contentType :
     *            type of the blob content (default : 'application/binary')
     */
    public function loadBlob($adresse, $contentType = 'application/binary') {
        ...
        if (! $fp)
            throw new Exception ( 'error loading the file' );
    ...
    public function sendRequest() {
        if (! $this->blobList) {
            ...
            if ($this->headers !== null) {
                $this->headers .= "Content-Type: application/json+nxrequest";
                $this->headers = array (
                        $this->headers,
                        $this->HEADER_NX_SCHEMAS
                );
